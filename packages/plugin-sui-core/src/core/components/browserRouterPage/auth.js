...
const getAuthorizedComp = (
    ...
    return (
      <ScrollContext.Consumer>
        {typeof wrapperType === 'string'
          ? ({ allowScroll = true }) =>
              wrapperType === 'Content' ? (
                <Content allowScroll={allowScroll}>{authorizedComp}</Content>
              ) : (
                <NarrowContent allowScroll={allowScroll}>
                  {authorizedComp}
                </NarrowContent>
              )
          : () => {
  ...
  // 若存在auth属性，则执行认证方法
  return auth
    ? auth(props.match).then(response => (response ? getCompWrapper() : null))
    : getCompWrapper();
};

export default getAuthorizedComp;
